...
def find_apps_with_j2_jobs(apps, jobs, t2_cutoff):
    ...
    for (u_id, job_id) in apps:
        ...
        if read_date(jobs[job_id][-1][:19]) >= t2_cutoff:
            try:
                apps_with_t2_jobs[u_id].append(job_id)
            except KeyError:
                apps_with_t2_jobs[u_id] = [job_id]
...
def get_similarity_score(u2_user, user):
    ...
    return current_job_title_score + degree_score + major_score + old_job_titles_score + (location_score + total_exp_score + managed_others_score + grad_year_score + work_history_count_score + currently_emp_score + managed_howmany_score)/9.0

def get_similar_users(u2_id, users_features, k):
    similar_user_ids = {}
    
    u2_user = users_features[u2_id]

    for u_id in users_features:
...
def predict_jobs_for_users(users2, users_features, apps_t2_jobs, jobs, k):
    ...
    count_users_with_jobs = count_users_without_jobs = 0
    for u2_id in users2:
        similar_users = get_similar_users(u2_id, users_features, k)
        for (u2_id, u_id) in similar_users:
            try:
                job_ids = apps_t2_jobs[u_id]
                count_users_with_jobs += 1
            except KeyError:
                job_ids = []
                count_users_without_jobs += 1
            for j_id in job_ids:
    ...
    print "found users in apps = %s, didnot find %s, users2 len %s" %(count_users_with_jobs, count_users_without_jobs, len(users2))
    return predictions
